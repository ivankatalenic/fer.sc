...
int main(int argc, char* argv[]) {
	if (argc != 7) {
		throw std::length_error("the number of expected arguments is 6");
	}

	const int left			{atoi(argv[1])};
	const int right			{atoi(argv[2])};
	const int left_angled	{atoi(argv[3])};
	const int right_angled	{atoi(argv[4])};
	const int speed			{atoi(argv[5])};
	const int direction		{atoi(argv[6])};
